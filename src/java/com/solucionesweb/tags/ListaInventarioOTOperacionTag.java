...
public class ListaInventarioOTOperacionTag extends TagSupport
    ...
    private void inicializarVariablesDeRetorno() {
        ...
        pageContext.setAttribute("nombreTerceroVar", fachadaPluFicha.getNombreTercero());
        pageContext.setAttribute("numeroOrdenVar", fachadaPluFicha.getNumeroOrdenStr());
        pageContext.setAttribute("referenciaClienteVar", fachadaPluFicha.getReferenciaCliente());
        pageContext.setAttribute("existenciaCantidadVar", fachadaPluFicha.getExistenciaCantidadDf1());
        pageContext.setAttribute("existenciaPesoVar", fachadaPluFicha.getExistenciaPesoDf1());
        pageContext.setAttribute("idOperacionVar", fachadaPluFicha.getIdOperacionStr());
        pageContext.setAttribute("nombreOperacionVar", fachadaPluFicha.getNombreOperacion());
        pageContext.setAttribute("nombreItemVar", fachadaPluFicha.getNombreItem());
        pageContext.setAttribute("nombreOperarioVar", fachadaPluFicha.getNombreOperario());
    ...
    public int doStartTag() throws JspTagException {
        ...
        colaboraOrdenTrabajo.setIdBodega(getIdOperacionTag());

        //
        Vector vectorBean = new Vector();

        //
        if (getIdOperacion() == xIdOperacionTerminados) {

            //
            vectorBean = colaboraOrdenTrabajo.listaOTTerminados();

        } else // NO filtra idplu = 209   
        if (getIdOperacion() == xIdOperacionExtrusion) {

            //
            vectorBean = colaboraOrdenTrabajo.listaOTUnOperacionExtrusion();

        } else {

            // SI filtra idplu = 209    
            vectorBean = colaboraOrdenTrabajo.listaOTUnOperacion();
