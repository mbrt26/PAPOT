package co.linxsi.controlador.touch.retal;
...
@WebServlet(name = "TouchRetalServlet", urlPatterns = {"/TouchRetalServlet"})
public class TouchRetalServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (xOpcion.equals("printRetales")) {
                printListaRetales(out);
            }
            if (xOpcion.equals("printRetalesDesplegable")) {
                printListaRetalDesplegable(out);
            }
    ...
    private void printListaRetales(PrintWriter out) {
        Touch_Retal_DAO dao = new Touch_Retal_DAO();
        dao.setSk_proceso(Integer.parseInt(sk_proceso));
        List<Touch_Retal_DTO> Lista = dao.getListScrapProcess();
        ...
        for (Touch_Retal_DTO t : Lista) {
            out.println(" <li class=\"list-group-item list-group-item-warning list-group-item-action\"   > "
                    ...
                    + "<input id = \"radioButtonR" + i + "\" onclick=\"copiarRetal(" + i + ")\" " + estado + " name = \"radioButtonR\" type=\"radio\" style=\"cursor:pointer;\"  value =\"" + t.getSk_retal() + "\">\n"
                    + "</div>\n"
                    + " <input id = \"inputR" + i + "\" value=\"" + t.getNombre_retal() + "\" hidden=\"true\"> <label type=\"text\" class=\"form-control form-control-warning bg-transparent border-0 text-md-left\" >"
                    + t.getNombre_retal()
                    + "</label>\n"
                    + "</div>\n"
                    + "</li>\n");
    ...
    private void printListaRetalDesplegable(PrintWriter out) {
        Touch_Retal_DAO dao = new Touch_Retal_DAO();
        dao.setSk_proceso(Integer.parseInt(sk_proceso));
        List<Touch_Retal_DTO> lista = dao.getListScrapProcess();
        out.println("<select id=\"listaRetal\" title =\"Retal\" class=\"custom-select col-md-3\"\">\n");

        for (Touch_Retal_DTO retal : lista) {
            out.println("<option value=\"" + retal.getSk_retal() + "\">" + retal.getNombre_retal() + "</option>\n");
...
//            i++;
    ...
    private boolean ingresarRetal(String xNumeroOrdenStr, String xPesoPerdido, String xIdOperario,
            String xIdOperacion, String xIdCausa, String comentario,
            String idMaquina, String turno, String fechaProduccion, HttpServletRequest request) {
...
//        String xIdOperacion = request.getParameter("ixProceso");
        ...
        dctoOrdenProgresoBean.setIdControlTipo(idMaquina);

        //---
        boolean res = dctoOrdenProgresoBean.ingresaInicio();
